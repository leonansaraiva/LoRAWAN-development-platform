...
#if LMIC_DEBUG_LEVEL > 0
    printf("\r%lu: Received downlink, window=%s, port=%d, ack=%d\r\n", os_getTime(), window, port, ackup);
#endif
    return 1;
}
#endif//DISABLE CRYPTOGRAPHY
    ...
    if( dlen > LEN_JA ) {
        ...
        for( u1_t chidx=3; chidx<8; chidx++, dlen+=3 ) {
            ...
            if( freq ) {
                ...
                printf("\r%lu: Setup channel, idx=%d, freq=%lu\r\n", os_getTime(), chidx, (unsigned long)freq);
#endif
            }
        }
    }
...
static void buildDataFrame (void) { 
    ...
    os_copyMem(LMIC.frame, LMIC.pendTxData, dlen);       
    LMIC.dataLen = dlen;
}
#else
static void buildDataFrame (void) {
...
static void engineUpdate (void) {
#if LMIC_DEBUG_LEVEL > 0
    printf("\r%lu: engineUpdate, opmode=0x%x\r\n", os_getTime(), LMIC.opmode);
#endif
